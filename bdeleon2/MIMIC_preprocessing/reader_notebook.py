...
class MIMICReader(object):
    ...
    def __init__(self, data_path, device=None, labs_only=False, no_labs=False,sample=0):
        self._labels_path = data_path + '/labels_notebook.csv'
        self._flat_path = data_path + '/flat_notebook.csv'
        self._timeseries_path = data_path + '/timeseries_notebook.csv'
        self._device = device
        ...
        self.labels = pd.read_csv(self._labels_path, index_col='patient')[sample:sample+1]
        self.flat = pd.read_csv(self._flat_path, index_col='patient')[sample:sample+1]
        ...
        # timdf = pd.read_csv(self._timeseries_path)
        # g=timdf.groupby('patient').size().sort_values(ascending=True)
        # print(g.head(8))
    ...
    def batch_gen(self, batch_size=1, time_before_pred=5):
