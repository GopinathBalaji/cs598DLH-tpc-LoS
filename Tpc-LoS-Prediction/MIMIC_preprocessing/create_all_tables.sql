...
\copy (select subject_id as uniquepid, hadm_id as patienthealthsystemstayid, stay_id as patientunitstayid, hospital_expire_flag as actualhospitalmortality, los as actualiculos from ld_labels as l where l.stay_id in (select * from ld_timeseries_patients) order by l.stay_id) to 'D:/UKR/TPC-LoS-prediction/MIMIC_data_preprocessed/labels.csv' with csv header
\copy (select * from ld_flat as f where f.patientunitstayid in (select * from ld_timeseries_patients) order by f.patientunitstayid) to 'D:/UKR/TPC-LoS-prediction/MIMIC_data_preprocessed/flat_features.csv' with csv header
\copy (select * from ld_timeserieslab as tl order by tl.patientunitstayid, tl.labresultoffset) to 'D:/UKR/TPC-LoS-prediction/MIMIC_data_preprocessed/timeserieslab.csv' with csv header
\copy (select * from ld_timeseries as t order by t.patientunitstayid, t.chartoffset) to 'D:/UKR/TPC-LoS-prediction/MIMIC_data_preprocessed/timeseries.csv' with csv header